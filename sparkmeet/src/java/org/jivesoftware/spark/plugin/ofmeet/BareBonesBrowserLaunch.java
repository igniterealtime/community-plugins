...
public class BareBonesBrowserLaunch {
   ...
   public static SimpleFrameExample browser = null;
   ...
   public static void openURL(String server, String port, String roomId)
   {
	 String baseUrl = server + ":" + port + "/ofmeet/?r=" + roomId;

	 Log.warning("BareBonesBrowserLaunch " + baseUrl);
		...
		if (osName.startsWith("mac os"))
		{
		   Class.forName("com.apple.eio.FileManager").getDeclaredMethod("openURL", new Class[] {String.class}).invoke(null, new Object[] {url});
			...
			try {
				...
				if (browser == null)
					browser = new SimpleFrameExample(url, false, false);
				else
					browser.setUrl(url);

			} catch (Exception e) {
				Log.warning("SimpleFrameExample: error ", e);
