var pdf = null;
var page = 1;
var pdfDoc = urlParam("pdf");
var control = urlParam("control");

function start()
{
	PDFJS.workerSrc = 'src/worker_loader.js';
	if (window.location.hash) page = window.location.hash.substring(1);
	if (pdfDoc) goto(page);
}

function goto(page)
{
	PDFJS.getDocument('/ofmeet/proxy?url=' + pdfDoc).then(function(p) {
		pdf = p;
		doPage(page, true);		
		window.parent.xmppConnection.ofmeet.pdfReady();		
	});
}
...
function nextPage()
{
	if (pdf && control == "true")
	{
		//console.log('next page', page, pdf.numPages);
		page++;
...
function doPage(pdfPage, push)
{
	if (pdf)
	{
	  ...
	  window.parent.xmppConnection.ofmeet.pfdGoto(pdfPage);
