...
$(document).ready(function() 
{
	...
	{
		setUpUser(window.location.host);
		...
		if (window.localStorage["store.settings.server"])
		{
			...
			console.log("authentication: found", server, username);	
			...
			if (server && username && password)
			{
				...
				xhr.onreadystatechange = function() 
				{
					...
					if (xhr.status >= 400)
					{
						console.error('authentication: bad username or password');
					}
				};
				xhr.open("GET", "https://" + server + '/ofmeet/config', true, username, password);		
				xhr.send();   

			} else console.error('authentication: bad configuration');
			...
			if (user)
			{
				console.log("authentication: ok using localStorage", user);		
				doIt();        

			}  else console.error('authentication: bad configuration');

		    } else {
			console.log("authentication: ok using javascript", user);		
			doIt();	    
		    }
		}
	}
