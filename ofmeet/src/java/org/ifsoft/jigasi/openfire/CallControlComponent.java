 ...
 */
package org.ifsoft.jigasi.openfire;
...
{
    private static final Logger Log = LoggerFactory.getLogger(CallControlComponent.class);
	...
	public CallControlComponent(File pluginDirectory)
	{
		...
		String port = JiveGlobals.getProperty("org.jitsi.videobridge.sip.port.number", "5030");
	...
	public CallSession findCreateSession(String from, String to, String destination)
	{
		...
		if (confJID != null && conferences.containsKey(confJID))
		{
			...
			Log.info("CallControlComponent - found conference id " + confJID + " " + confId);
		...
		if (confName != null && conference == null)
		{
			for (Conference conf : getVideobridge().getConferences())
			{
				if (conf.getName() != null && conf.getName().equals(confName))
				{
					conference = conf;
					break;
				}
		...
		if (conference != null)
		{
			Log.info("CallControlComponent - found conference " + conference.getFocus());
