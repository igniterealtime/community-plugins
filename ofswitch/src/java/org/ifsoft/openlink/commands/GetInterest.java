...
public class GetInterest extends OpenlinkCommand {
	...
	public GetInterest(OpenlinkComponent traderLyncComponent) {
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			{
				OpenlinkUserInterest traderLyncUserInterest = this.getOpenlinkComponent().getOpenlinkInterest(interestID);
				...
				if (traderLyncUserInterest != null)
				{
					...
					if (traderLyncUserInterest.getCalls().size() > 0)
					{
						...
						while( it2.hasNext() )
						{
							...
							//this.getOpenlinkComponent().traderLyncLinkService.restoreCallState(traderLyncCall.line, traderLyncCall.callid); // we wait for Openlink responses
							Thread.sleep(1000);
