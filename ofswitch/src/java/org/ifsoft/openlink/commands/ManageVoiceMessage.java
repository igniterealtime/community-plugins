...
public class ManageVoiceMessage extends OpenlinkCommand
{
	public ManageVoiceMessage(OpenlinkComponent traderLyncComponent)
	{
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			String action = in.element("action").getText();
			...
			String errorMessage = this.getOpenlinkComponent().manageVoiceMessage(newCommand, profileID, featureID, action, value1);

			if (errorMessage != null)
			{
				Element note = newCommand.addElement("note");
				note.addAttribute("type", "error");
				note.setText("Manage Voice Message Error " + errorMessage);
			}

		}catch(Exception e)
		{
			Log.error("[OpenlinkOpenlink] Manage Voice Message execute error " + e);

			Element note = newCommand.addElement("note");
			note.addAttribute("type", "error");
			note.setText("Manage Voice Message Internal error");
		}
		return newCommand;
	}//end execute
	...
	{
		return "http://xmpp.org/protocol/openlink:01:00:00#manage-voice-message";
	}
