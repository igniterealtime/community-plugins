...
public class AuthFilter implements ContainerRequestFilter {
	...
	/** The plugin. */
	private ChatApiPlugin plugin = ChatApiPlugin.getInstance();
	...
	public ContainerRequest filter(ContainerRequest containerRequest) throws WebApplicationException {
		...
		if ("basic".equals(plugin.getHttpAuth())) {
			...
			boolean adminOnly = JiveGlobals.getBooleanProperty("chatapi.restapi.adminonly", true);

			if (adminOnly && !containerRequest.getPath().startsWith("restapi/v1/chat"))
			{
