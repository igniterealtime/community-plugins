<%@ page
	import="java.util.*,
                ...
                org.jivesoftware.util.*,org.jivesoftware.openfire.plugin.rest.ChatApiPlugin,
...
<%
	...
	ChatApiPlugin plugin = (ChatApiPlugin) XMPPServer.getInstance().getPluginManager()
			.getPlugin("chatapi");
	...
	if (save) {
		...
		if (errors.size() == 0) {
			...
			if(is2Reload) {
				...
				String pluginDir = pluginManager.getPluginPath( plugin ).toFile().getName();
			...
			<div>
				...
				<ul>
					...
					<label for="http_basic_auth">HTTP basic auth - REST API
						authentication with Openfire user account.</label>
					<br>
